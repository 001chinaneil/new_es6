...
function FileProgress(file, targetID) {
  ...
  if (!this.fileProgressWrapper.length) {
    ...
    var progressBar = $("<div/>");
    progressBar.addClass("progress-bar progress-bar-info").attr('role', 'progressbar').attr('aria-valuemax', 100).attr('aria-valuenow', 0).attr('aria-valuein', 0).width('0%');
...
FileProgress.prototype.setChunkProgess = function (chunk_size) {
  ...
  for (var i = 1; i <= chunk_amount; i++) {
    ...
    var progressBar = $("<div/>");
    progressBar.addClass("progress-bar progress-bar-info text-left").attr('role', 'progressbar').attr('aria-valuemax', 100).attr('aria-valuenow', 0).attr('aria-valuein', 0).width('0%').attr('id', this.file.id + '_' + i).text('');
...
FileProgress.prototype.bindUploadCancel = function (up) {
  ...
  if (up) {
    self.fileProgressWrapper.find('td:eq(2) .progressCancel').on('click', function () {
      ...
      //增加自定义业务逻辑 取消正在上传的文件后 "上传音频"按钮的遮罩去掉
      $(".uploadAudioBoxShade").hide();
