 ...
 * 删除图片库的图片 and 24小时视频
 */
$('body').on('click', '.js-delect-pic, .js-delete-moment-video', function (e) {
    var t = $(this);
    if (t.attr("data-type") !== 'feededit' && t.attr('data-type')!=='feedadd') t.parents('li').remove();
    $('.pic-number').html($('.js-pic-list li').length);
    ...
    ;
    //24视频
    if (t.hasClass('moment-video-del')) {
        t.parents('.js-moment-video-pic').attr({'data-vid': '', 'data-videoId': ''});
        t.parents('.js-moment-video-pic').find('img').attr('src', '');
        t.parents('.box-info-wrap').find('div.js-open-video-modal').removeClass('hide');
        $('#moment-upload-img').attr('type', 'file');
    }

    //feed流广告
    if(t.hasClass('delete-ad-img')){
        var parent = t.parent(),staticUrl = $("#staticUrl").val();
        parent.find('#MomentAd').attr('src','');
        parent.find('#img_urls').attr({
            'data-type':'',
            'data-imgid':'',
            'data-imgurl':''
        }).val('');
        t.hide();
        if($('#img_uuid').length>0) $('#img_uuid').val('');
        $('.js-upload-img').css('background','url('+staticUrl+'/admin/image/bg.png) center no-repeat');
        return false;
    }
});
/**
    ...
    current_image.onload = function () {
        ...
        if (file.type == 'image/gif') {
            ...
            if (this.width < _pic.width) {
                ...
                Messenger().post({
                    message: '图片尺寸不符合要求,请上传' + _pic.width + '*' + _pic.height + '尺寸的图片',
                    type: 'error',
                    showCloseButton: true
                });
            }
...
var getMomentVideoList = function (page, callback) {
    $.ajax({
        type: 'post',
        url: '/moment/getVideoList',
        data: {
            page: page
        },
        dataType: 'json',
        ...
        success: function (data) {
            if (data.result == 1) {
                var liHtml = '';
                var p = 0;
                $.each(data.data.datalist, function (index, video) {
                    var img = video.cover_path ? video.cover_path : '/static/common/images/bg.png';
                    liHtml +=
                        '<li data-id="' + video.video_id + '">' +
                        '<a>' + interceptionString(video.title) + '</a>' +
                        '<input type="radio" class="custom" name="search-video">' +
                        '<img class="history-img"  src="' + img + '" >' +
                        '</li>';
                });
            ...
            if (data.result == 1) {
                ...
                $.each(data.data, function (index, video) {
                    liHtml +=
                        ...
                        '<a>' + interceptionString(video.title) + '</a>' +
                        '<input type="radio" class="custom" name="search-video">' +
                        '<img class="history-img"  src="' + video.cover + '" >' +
                        '</li>';
                });
...
$('body').on('click', '.js-get-more-search-video', function () {
    var t = $(this);
    if ($('#admin-moment-add-box').length > 0) {
        getMomentVideoList(t.attr('data-page'));
    } else {
